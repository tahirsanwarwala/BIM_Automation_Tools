...
Provides standard outer diameters (OD) and nominal trade sizes for
Electrical Conduit (EMT) in decimal feet (Revit internal units).

This is a standalone copy used by the external processing engine.
The pyRevit lib copy mirrors this file.
"""
...
def snap_to_trade_size(raw_radius_ft):
    """
    Snap a raw scan-fitted radius (decimal feet) to the nearest
    standard EMT conduit trade size.
    ...
    Returns:
        dict with keys:
            label        (str):   e.g. '3/4"'
            nominal_ft   (float): Nominal diameter in feet
            od_ft        (float): Outer diameter in feet
            delta_in     (float): Abs difference raw OD vs trade OD in inches
    """
