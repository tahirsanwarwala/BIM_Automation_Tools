...
picked linked walls; arched window heads; curtain wall type matching by Type
Mark; splitting a curtain wall at a level.
...
anything already out of scope in V1.
...
**`lib/Tahir/arch_fit.py`** — PURE geometry, no Revit import, unit-tested
outside Revit like `wall_bands`. Lifted from `WindowToCurtainWall`, where it
has no tests at all today; giving it some is the one genuine improvement
this extraction buys beyond removing duplication.

    solve3, fit_circle, fit_circle_trimmed, simplify_chain

**`lib/Tahir/window_cw.py`** — Revit-facing window work, from
`WindowToCurtainWall`: measuring a window (`length_param`, `window_centre`,
`sill_from_parameters`, `measure_window`), tracing and building an arched
head (`trace_top_boundary`, `plane_point`, `close_profile`,
`build_arch_profile`, `profile_extent`), matching a curtain wall type
(`type_mark`, `mark_letters`, `mark_prefix`, `candidate_prefixes`,
`curtain_wall_types`, `match_curtain_type`, `prompt_curtain_type`), and
placing one (`wall_axis`, `segment_on_wall`, `strip_curtain_grid`).
...
Per picked wall, alongside V1's measurement:
  ...
  * Each window measured by `window_cw.measure_window`: sill from
    ...
    back to Width/Height, the head traced from the linked geometry.
...
An arched head is NOT applied here — see Phase 5c. Revit embeds the curtain
wall in the skin wall it overlaps. Note the skin wall is a finish layer,
commonly 2-3 inches, not a full compound wall.

### Phase 5c — every profile edit, after the transaction closes

Two kinds of profile edit happen here, and both must be here rather than in
Phase 5b:

  * an arched head re-sketched onto a curtain wall — on the TOP piece only,
    since the lower pieces of a window split at a level are rectangular by
    construction;
  * a rectangular `Opening` cut out of a skin wall's elevation.

Both go through `wall_sketch.apply_profile`, and both run LAST, outside the
main transaction, for two reasons that leave no choice: `SketchEditScope`
refuses to start inside an open transaction — it opens transactions of its
own — and a sketched profile is drawn against the wall's constraints, so
those must already be final. `WindowToCurtainWall` sequences it the same way
...
| Arched head on a split window | Arch on the top piece; lower pieces rectangular |
...
Unit tests, run outside Revit with `python -m unittest discover -s tests`:
  ...
  * `tests/test_arch_fit.py` is new, covering the extracted pure geometry:
    `solve3` on a known system and on a singular one; `fit_circle` on three
    points of a known circle and on collinear points; `fit_circle_trimmed`
    against an outlier; `simplify_chain` on a straight run, on a curve, and
